...
int hashTable::insert(const std::string &key, void *pv)
{
    ...
    // previous version was not handeling lazily deleted items correctly. Fixed issue
	while ((data[hashVal].isOccupied || data[hashVal].isDeleted) && (data[hashVal].key != key))
    {
		++hashVal %= capacity;
	}
	// if key already exists in hash table, return 1
	if (data[hashVal].key == key && data[hashVal].isDeleted == false)
    {
		return 1;
	}
...
//remove, getpointer, setpointer are added functions for heap functionality
//get pointer of the key
void *hashTable::getPointer(const string &key, bool *b)
{
    int pos = findPos(key);
    //if key is found, set b to be true. Otherwise, set to false
    if (b != NULL)
    {
        *b = (pos != -1);
    }
    //return null pointer if key is not found
    if (pos == -1)
    {
        return nullptr;
    }
    //return pointer assosciated with the specific key
    else
    {
        return data[pos].pv;
    }
...
//set the pointer associated with the key. Return 0 on success, 1 if it does not exist in the hash table
...
//delete the item with specificed key, return true on success and false if the key does not exist in the hash table
